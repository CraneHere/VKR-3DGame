...
void Application::MainLoop()
{
    ...
    while (m_Running && !s_Window->ShouldClose())
    {
        ...
        ImGui::Begin("Atmospheric Settings");
        
        // Directional Light
        if (ImGui::CollapsingHeader("Directional Light", ImGuiTreeNodeFlags_DefaultOpen))
        {
            auto& light = s_Game->GetDirectionalLight();
        ...
        {
            auto& ambient = s_Game->GetAmbientLight();
            ImGui::ColorEdit3("Sky Color", &ambient.skyColor.r);
            ImGui::ColorEdit3("Ground Color", &ambient.groundColor.r);
            ImGui::SliderFloat("Ambient Intensity", &ambient.intensity, 0.0f, 1.0f);
        }

        // Fog
        if (ImGui::CollapsingHeader("Fog", ImGuiTreeNodeFlags_DefaultOpen))
        {
            auto& fog = s_Game->GetFogSettings();
            ImGui::Checkbox("Enabled", &fog.enabled);
            ImGui::ColorEdit3("Fog Color", &fog.color.r);
            ImGui::SliderFloat("Density", &fog.density, 0.0f, 0.1f, "%.4f");
