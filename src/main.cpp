        ...
        while (SDL_PollEvent(&event))
        {
            ...
            if (event.type == SDL_MOUSEBUTTONDOWN && mouseCaptured && !io.WantCaptureMouse && !editorMode) {
                ...
                if (event.button.button == SDL_BUTTON_LEFT) {
                    player.odmGear.fireLeftGrapple(physicsSystem.scene, player.position, forward, right, player.horizontalSpeed);
                }
                else if (event.button.button == SDL_BUTTON_RIGHT) {
                    player.odmGear.fireRightGrapple(physicsSystem.scene, player.position, forward, right, player.horizontalSpeed);
                }
            }
