...
bool Game::Initialize()
{
    ...
    std::cout << "=== Initializing Atmospheric Graphics ===" << std::endl;
    ...
    m_Initialized = true;
    std::cout << "=== Atmospheric Graphics Initialized ===" << std::endl;
...
{
    // Основной шейдер с атмосферным освещением
...
void Game::CreateGeometry()
{
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_WallEBO);
...
void Game::RenderShadowPass()
{
    ...
    // Отключаем face culling для теней (избегаем артефактов)
    glCullFace(GL_FRONT);
...
void Game::RenderMainPass()
{
    ...
    m_MainShader.SetVec3("uObjectColor", 0.4f, 0.35f, 0.3f); // Землистый цвет
    glDrawElements(GL_TRIANGLES, FLOOR_INDEX_COUNT, GL_UNSIGNED_INT, 0);

    // === Рендерим кубы (здания) ===
    glBindVertexArray(m_CubeVAO);
    for (int x = -2; x <= 2; x++)
    {
        for (int z = -2; z <= 2; z++)
